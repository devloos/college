...
def check_unique_constraint(session: Session, constraint, instance):
    ...
    count = session.query(python_class).filter(
        *filters).count()  # See if we have any matches
...
def check_unique(sess: Session, class_instance) -> list[str]:
    ...
    constraints = [{'name': primary_key['name'],
                    'column_names': primary_key['constrained_columns']}]
