...
def add_department(session: Session):
    ...
    while not unique_abbreviation or not unique_name:
        ...
        name_count: int = session.query(Department).filter(
...
def delete_department(session: Session):
    ...
    n_courses = session.query(Course).filter(
...
def list_department(session: Session):
    ...
    departments: [Department] = list(session.query(
        Department).order_by(Department.abbreviation))
...
def list_student(sess: Session):
    ...
    :return:
    """
    students: [Student] = list(sess.query(Student).order_by(
...
def list_student_major(sess: Session):
    ...
    for stu in recs:
        print(
            f"Student name: {stu.lastName}, {stu.firstName}, Major: {stu.name}, Description: {stu.description}")
...
def select_student_from_list(session):
    ...
    students: [Department] = list(sess.query(Department).order_by(
        Department.lastName, Department.firstName))
    ...
    for student in students:
        ...
        options.append(Option(student.lastName + ', ' +
    ...
    returned_student = sess.query(Department).filter(
        Department.studentId == int(text_studentId)).first()
...
def boilerplate(sess):
    ...
    major2: Major = Major(department, 'Computer Engineering',
                          'Much closer to the silicon')
