 ...
 * push
 ----------------------------------------------------- 
 * Pushes a new person on to the stack
 ...
 * to peek at" else we output person on top of stack
 * RETURNS - void
 ****************************************************/
 ...
 * The program prompts user with a menu choices are Push a person on the
 * stack, pop a person off the stack, peek at the top of the stack, is empty
...
void displayMenu()
{
    ...
    cout << "3 - Is the stack empty?  (ISEMPTY)\n";
    ...
    int &size)          //var stores size of stack
{
    ...
    {
        case PUSH:
            //PROCCESSING/IO - calls push function to add person to the stack
            push(&head, size);
            break;
        case POP:
    ...
    int &size)     //int stores size of stack
{
    ...
    //PROCCESSING - node.nextNode to = head and assigning head to = node, incrementing size
    node->nextNode = *head;
    *head = node;
    ++size;
}
    ...
    Person** head) //ptr to a ptr stores head of stack
{
    ...
    //PROCCESSING - if node is empty then print nobody to peek else ouputs person on top of stack
    if (isEmpty(&node))
...
void heading() 
{
	...
	const int LAB_NUM       = 8;
	const char LAB_NAME[]   = "Lab Implementing a Stack";
